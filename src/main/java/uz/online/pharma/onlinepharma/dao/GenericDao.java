package uz.online.pharma.onlinepharma.dao;
...
public abstract class GenericDao<T extends Auditable, C extends GenericCriteria> implements BaseRepository {
    ...
    public <G> G find(Long id, Class<G> object) {
        try {
            return entityManager.createQuery("Select t from " + object.getSimpleName() + " t where not t.deleted and t.id = " + id, object).getSingleResult();
        } catch (NoResultException ex) {
            return null;
        }
    }
    ...
    protected <G> List<G> getResults(C criteria, Query query) {
        if ((criteria.getPage() == null || criteria.getPerPage() == null)
                || (criteria.getPage() < 0 || criteria.getPerPage() <= 0)) {
